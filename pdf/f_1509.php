...
$servidor='';
  if($nomservidor!=''){
  	$servidor.='Nombre: '.$nomservidor;
  }
  if($cargoservidor!=''){
  	$servidor.='   Cargo: '.$cargoservidor;
  }
  if($indpersona!=''){
  	$servidor.='   Descripcion: '.$indpersona;
  }
...
$pdf->rectangle (puntos_cm(1), puntos_cm(22.3), puntos_cm(19.1), puntos_cm(0.6));
$pdf->addText(puntos_cm(1.3),puntos_cm(22.5),9,'LUGAR DONDE OCURRIO EL HECHO: '.$lugar);
...
$pdf->rectangle (puntos_cm(1), puntos_cm(14.3), puntos_cm(19.1), puntos_cm(0.6));
$pdf->addText(puntos_cm(1.1),puntos_cm(14.5),8,'PRUEBAS DE LA QUEJA.');
$pdf->addText(puntos_cm(4.5),puntos_cm(14.5),7,utf8_decode('(Sólo en caso de presentar una queja, por favor indicar las pruebas que presenta para respaldo).'));
$pdf->rectangle (puntos_cm(1), puntos_cm(11.9), puntos_cm(19.1), puntos_cm(2.3));
$pdf->addText(puntos_cm(1.3),puntos_cm(13.9),7,utf8_decode($pruebas));//pruebas quejas

$pdf->rectangle (puntos_cm(1), puntos_cm(10.8), puntos_cm(19.1), puntos_cm(1));
$pdf->addText(puntos_cm(1.1),puntos_cm(11.4),8,utf8_decode('IDENTIFICACIÓN DEL/LA SERVIDOR/A PÚBLICO.'));
$pdf->addText(puntos_cm(7.8),puntos_cm(11.4),7,utf8_decode('(Sólo en caso de presentar una queja, por favor indique rasgos que permitan la individualización de la persona'));
$pdf->addText(puntos_cm(7.8),puntos_cm(11.0),7,utf8_decode(' vinculada al hecho).'));
$pdf->rectangle (puntos_cm(1), puntos_cm(9.6), puntos_cm(19.1), puntos_cm(1.1));
$pdf->addText(puntos_cm(1.3),puntos_cm(10.2),7,utf8_decode($servidor));//servidor publico

$pdf->rectangle (puntos_cm(1), puntos_cm(8.9), puntos_cm(8.9), puntos_cm(0.6));
$pdf->addText(puntos_cm(1.3),puntos_cm(9.1),10,'DATOS DEL/LA GENERADOR/A DE LA QUEJA');
$pdf->rectangle (puntos_cm(1), puntos_cm(7.8), puntos_cm(19.1), puntos_cm(1));
$pdf->addText(puntos_cm(1.3),puntos_cm(8.4),8,utf8_decode('IDENTIFICACIÓN.'));
$pdf->addText(puntos_cm(3.7),puntos_cm(8.4),7,utf8_decode('(Por favor indique sus datos personales para poder contactarlo en caso de requerir mayor información e/o informarle sobre el desarrollo del caso.'));
$pdf->addText(puntos_cm(1.3),puntos_cm(8.0),7,utf8_decode('También puede presentar éste formulario de forma ANONIMA y no ingresar ningún dato)'));
$pdf->addText(puntos_cm(1.3),puntos_cm(7.4),8,utf8_decode('Nota: El tratamiento y acción derivada de la presente queja, será respondida formalmente mediante la respectiva nota de respuesta de acuerdo a los '));
$pdf->addText(puntos_cm(1.3),puntos_cm(7.1),8,utf8_decode('datos que el/la denunciante proporcione. '));
if($anonimo=='SI')
{
    $pdf->addText(puntos_cm(3.8),puntos_cm(6.4),9,'Nombre:');
    $pdf->addText(puntos_cm(1.5),puntos_cm(5.9),9,'Documento de identidad:');
    $pdf->addText(puntos_cm(2.6),puntos_cm(5.4),9,utf8_decode('Teléfono/Celular:'));
    $pdf->addText(puntos_cm(2.3),puntos_cm(4.9),9,utf8_decode('Correo electrónico:'));
    $pdf->addText(puntos_cm(3.6),puntos_cm(4.4),9,utf8_decode('Dirección:'));
}
else 
{
    ...
    $pdf->addText(puntos_cm(2.6),puntos_cm(5.4),9,utf8_decode('Teléfono/Celular: ').$tel);
    $pdf->addText(puntos_cm(2.3),puntos_cm(4.9),9,utf8_decode('Correo electrónico: ').utf8_decode($email));
    $pdf->addText(puntos_cm(3.6),puntos_cm(4.4),9,utf8_decode('Dirección: ').utf8_decode($direccion));
}

$pdf->addText(puntos_cm(14),puntos_cm(5.9),9,utf8_decode('¿Mantener en reserva su identidad?'));
if($reserva=="SI")
{
