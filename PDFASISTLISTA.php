...
class PDF extends FPDF
{
    ...
    function cabeceraHorizontal()
    {
        ...
        $ejeX = 20;
		$this->RoundedRect($ejeX, 45, 60, 7, 2, 'FD');
		$this->CellFitSpace(60,7, 'NOMBRES',0, 0 , 'C');
		$ejeX = $ejeX + 60;
		$this->RoundedRect($ejeX, 45, 30, 7, 2, 'FD');
		...
		$ejeX = $ejeX + 30;
		$this->RoundedRect($ejeX, 45, 30, 7, 2, 'FD');
		$this->CellFitSpace(30,7, 'TOTAL ATRASOS',0, 0 , 'C');
		$ejeX = $ejeX + 30;
	
    }
	
 
    function datosHorizontal($datos,$total,$ejeY,$datosp)
    {
        $this->SetXY(20,$ejeY);
        $this->SetFont('Arial','',8);
        $this->SetFillColor(229, 229, 229); //Gris tenue de cada fila
        $this->SetTextColor(3, 3, 3); //Color del texto: Negro
       // $ejeY = 52; //Aquí se encuentra la primer CellFitSpace e irá incrementando
        ...
        for($j=0;$j<count($datos);$j++)
		{
			...
			$t2=explode(' ',$t[0]);
			$ejeX = 20;
			$this->RoundedRect($ejeX, $ejeY, 60, 7, 2, 'FD');
			$this->CellFitSpace(60,7, $datosp,0, 0 , 'C');
			$ejeX = $ejeX + 60;
			$this->RoundedRect($ejeX, 45, 30, 7, 2, 'FD');
			$this->CellFitSpace(30,7, $t2[0],0, 0 , 'C');
			$ejeX = $ejeX + 30;
			$this->RoundedRect($ejeX, 45, 30, 7, 2, 'FD');
			$this->CellFitSpace(30,7, $t1[0],'LR', 0 , 'C');
			$ejeX = $ejeX + 30;
			$this->RoundedRect($ejeX, 45, 30, 7, 2, 'FD');
			$this->CellFitSpace(30,7, $t[1],0, 0 , 'C');
			$ejeX = $ejeX + 30;
			$this->RoundedRect($ejeX, 45, 30, 7, 2, 'FD');
			$this->CellFitSpace(30,7, $total,0, 0 , 'C');
			$ejeX = $ejeX + 30;
