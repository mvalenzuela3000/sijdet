<?php
	...
	if(verifica_cupos_institucion($_POST["institucion"],2)==0)
	{
		?>
	...
	/*?>
		...
		<script>
			alert("<?php verifica_cupos_institucion($_POST["institucion"],2);?>");
		</script>
	<?php*/
	...
	else
	{
		...
		$query="call pa_registro_becados('".$nombre."','".$apellido."','".$ci."',$ext,'".$email."',$dpto,'".$_SERVER['REMOTE_ADDR']."','".$fono."','$inst','".$otrainst."',2,'".$cod."',$pais)";
			  ...
			  $pass=substr($nombre, 0,1).substr($apellido,0,1).$ci;
		       // $link="http://www.ait.gob.bo/sisjornadas/formulario.php?c1=".base64_encode($ci)."&c0d=".base64_encode($cod).""; //para desarrolllo 
		        ...
		        $body = "<b>Inscripción a las ".$evento."</b><br><br>".
					...
					" Se confirmó el depósito realizado, para completar su inscripción agradeceremos ingrese o copie en su navegador el link  <strong>$link</strong> y posteriormente deberá imprimir el formulario de inscripción respectivo el mismo que deberá presentar al momento de su acreditación.".
		            "<br><br>Usuario: $ci<br><br>".
					"<br><br>Contraseña: $pass<br><br>";
		...
		<script language="javascript">
			...
			location.href = "../FormInscripcion.php"; 
		</script>
		<?php
		}
		else
