<?php
	...
	if(verifica_cupos_institucion(0,1)==0)
	{
		?>
		
		<script>
			history.back(-1);
			alert("Ya no hay cupos para inscribir.");
		</script>
		<?php
		exit;
	}
...
if($_POST["tipodep"]==0)
{
	...
	if($resultado = $conex->query($query))
	{
		...
		if($row[0]==0)
		{
			...
			<script language="javascript">
				history.back(-1);
				alert("El número de depósito ingresado aún no se verificó o no es correcto, si está seguro del número ingresado, por favor tome en cuenta que desde la fecha y hora de depósito deben transcurrir 24 horas para verificar el mismo con el banco antes de que se le habilite para inscribirse.");
			</script>
			<?php
			exit ;
		}
			...
			else
			{
				...
				if($resultado = $conex->query($query))
				{
				   ...
				   // $link="http://www.ait.gob.bo/sisjornadas/formulario.php?c1=".base64_encode($ci)."&c0d=".base64_encode($cod).""; //para desarrolllo 
					...
					$subject = "Constancia de Inscripción - XI Jornadas Bolivianas de Derecho Tributario.";
					$body = "<b>Inscripción a las XI Jornadas Bolivianas de Derecho Tributario</b><br><br>".
					...
					try {
					    require_once '../includes/codigo.php';
							...
							$mail->SMTPSecure = 'tls';
							$mail->Port       = 587;
							$mail->setFrom(USERMAIL,'XI Jornadas de Derecho Tributario Bolivia');
							if($email!="")
							{
							    $mail->addAddress($email, 'XI Jornadas de Derecho Tributario Bolivia - Inscripción');
							}
							$mail->addAddress($emailgaf, 'XI Jornadas de Derecho Tributario - Inscripción');
				...
				<script language="javascript">
					...
					location.href = "../FormInscripcionManual.php"; 
				</script>
				<?php
				}
				else
