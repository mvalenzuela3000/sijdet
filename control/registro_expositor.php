<?php
	...
	$ruta.="_".$ci;
	if(verifica_cupos_institucion(0,4)==0)
	{
		?>
		
		<script>
			history.back(-1);
			alert("Ya no hay cupos para expositores/autoridades.");
		</script>
		<?php
		exit;
	}
	...
	else
	{
		...
		$query="call pa_registro_becados('".$nombre."','".$apellido."','".$ci."',$ext,'".$email."',$dpto,'".$_SERVER['REMOTE_ADDR']."','".$fono."',0,'',4,'".$cod."',$pais)";
		if($resultado = $conex->query($query))
		{
			  $gestion=date('Y');
		        //$link="http://192.168.18.126/jornadas/formulario.php?c1=".base64_encode($ci)."&c0d=".base64_encode($cod).""; //para desarrolllo 
		        ...
		        $body = "<b>Inscripción a las ".$evento."</b><br><br>".
		        	"<b>Registro de Expositores / Autoridades</b><br><br>".
		...
		<script language="javascript">
			...
			location.href = "../FormRegistroExpositor.php"; 
		</script>
		<?php
		}
		else
